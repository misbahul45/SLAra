...
- **Data** (Go) — business logic, shipment/driver/vehicle/route, REST API
...
│   └── ai/                # FastAPI + ML
├── infra/                # docker-compose, kafka, environments
├── shared/               # events, contracts, utils (di-share via path)
...
- **Contract-first**: API/event contract ada di `shared/` + `docs/contracts/`. Breaking change wajib ADR (`docs/architecture/adr/`).
